...
- **"Phylum Path" shipped July 14, piloted on Phylum 1 only.** New `phylumPath` module — a bottom-up insight→article pipeline, direction-inverted from `proposeLineage`/`silentPropose`'s top-down one-shot placement. Spec'd via an 8-question questionnaire before coding (record in `/root/.claude/plans/woolly-watching-lamport.md`): both auto-detect + manual entry, depth flexes per insight, manual-button article regeneration, every rank gets its own article, persona extends Prod Oracle's 3-layer method. Also added `taxonomyTree.RANK_NAMES`/`rankNameForDepth()` — first live-app use of rank naming (Phylum/Order/.../Variant), previously display-only in `taxonomy_map.html`. Phylum 1's `_PHYLA_KEYWORDS` entry enriched 7→~140 terms same session to seed this pipeline.
- **Efficiency audit same session found + fixed 2 real issues**: `RPGACE.cache.clear()` had never actually busted anything (cache keys are `table+'|'+params`, but the insert/update/del wrappers called `clear(table)` with the bare table name, which never matched — stale reads could persist up to 60s after any write on any cached table); and Phylum Path's own auto-detect had installed a second, duplicate `MutationObserver` on `#send-btn` instead of reusing the existing shared phyla-scan observer — consolidated onto a new `RPGACE.hooks` event (`oracle:response-scanned`) instead.
- **Phylum 1 data repaired + tree built out from a 90-term jargon sweep, same session.** The questionnaire's "new insights only, no retroactive repair" decision was revisited when asked to "fix all of it" — phylum 1's 23 existing nodes had `parent_id: null` on every row (known un-migrated bug), now backfilled with zero orphans remaining. Also found and fixed a garbage-named leaf (a 298-char quest-plan blob stored as `name` instead of `deep_content`). `_generateNodeContent`'s prompt trimmed (5 sections/1500 tokens → 2 sections/900 tokens) on the theory that the empty-`deep_content` mystery is the same already-documented Oracle 504 timeout, just failing silently — not empirically confirmed. Built 32 new `taxonomy_tree` rows organizing all 90 jargon terms into coherent terminology-bucket leaves under the 5 existing Orders plus one new 6th Order ("Genre & Production Vocabulary"). **Corrected same session**: 2 of those leaves named specific artists (Dave x Central Cee, Endshpiel & Miyagi) — moved out of the tree into real `encyclopedia` entries (`source: 'taxonomy_case_study'`) linked back via `taxonomy_node_id` to the general concept node they illustrate, since the tree should classify general reusable concepts, not bake in one-off applied examples. Their former parent nodes flipped back to `node_type: 'leaf'`. Phylum 1 row count: 23 → 55 → 53. None of this hand-tested in the live app yet — done directly against Supabase, no browser access this session.
