...
- **"Phylum Path" shipped July 14, piloted on Phylum 1 only.** New `phylumPath` module — a bottom-up insight→article pipeline, direction-inverted from `proposeLineage`/`silentPropose`'s top-down one-shot placement. Spec'd via an 8-question questionnaire before coding (record in `/root/.claude/plans/woolly-watching-lamport.md`): both auto-detect + manual entry, depth flexes per insight, new insights only (no retroactive repair of phylum 1's existing flat-`parent_id` nodes), manual-button article regeneration, every rank gets its own article, persona extends Prod Oracle's 3-layer method. Real data check before building found phylum 1's 23 existing nodes all still have `parent_id: null` (known un-migrated bug) and `deep_content` empty on every one despite `_generateNodeContent` supposedly populating it — a second, previously-undiscovered bug, deliberately left uninvestigated per the questionnaire. Also added `taxonomyTree.RANK_NAMES`/`rankNameForDepth()` — first live-app use of rank naming (Phylum/Order/.../Variant), previously display-only in `taxonomy_map.html`. Phylum 1's `_PHYLA_KEYWORDS` entry enriched 7→~140 terms same session to seed this pipeline. Not yet hand-tested.
