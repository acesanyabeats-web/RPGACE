...
*Storage:* `beat-audio` bucket (Beat Log's librosa uploads, private, blanket `anon_all`); `beat-deliverables` bucket (Aug 5, Phase H — real sellable beat deliverable files/stems/zips, private, anon INSERT/DELETE only — deliberately no anon SELECT, since raw bytes here are Alex's paid IP, not disposable analysis samples; reads only via the service-role key inside `api/bundle-deliverables.js` or a signed URL it hands out).
...
- **Aug 5 block — Content/Video Pipeline unification, Phases A-H COMPLETE (the full confirmed 8-phase build), all `/Engineer`-disciplined, ALL code-verified/gate-passed/live-on-`main`, NONE hand-tested (Alex has explicitly deferred hand-testing until all 8 phases ship, verified together in one pass with a real beat).** Full spec + verbatim `/interrogation` answers: `content_video_pipeline_unification_spec_2026-08-05.txt`. Per-phase records: `engineer_pass_2026-08-05_01.txt` through `_09.txt`.
  ...
  - **Phase H (final phase)** — real beat deliverables per licence tier. New private `beat-deliverables` Storage bucket (anon INSERT/DELETE only, no SELECT — a deliberate, reasoned deviation from `beat-audio`'s blanket `anon_all` precedent, since this bucket holds Alex's real sellable stems, not disposable analysis samples); new `content_productions.deliverable_files`/`.deliverable_bundles` jsonb columns; new `api/bundle-deliverables.js` Vercel function server-side zipping a tier's applicable files with `archiver` (**this project's first-ever npm dependency, server-side/Vercel-function-only, never touches the client bundle**) and handing back a 24h signed download URL; a "📦 Manage Deliverables" button on ConID cards (same `licence_type` gate as the Beatstars listing button) supporting both individually-named files and a whole-folder drop. Real, reconfirmed constraint driving the whole design: BeatStars still has no upload/listing API — this stops at "auto-prepare a downloadable bundle," never a real BeatStars push. Real browser-automation autoport (Option 2, Alex named "cr4wl.ai" or another repo) is a confirmed, deliberately deferred future phase: `beat_deliverables_autoport_backlog_2026-08-05.txt`. The 8-phase build (A-H) is now complete — the next real milestone is Alex's one deferred full hand-test pass, not a new phase.
...
flowchart TD
    ...
    DELIVBTN --> BUNDLEGEN{{"🎁 Generate Bundle<br/>api/bundle-deliverables.js<br/>server-side zip, service-role key"}}
