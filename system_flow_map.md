...
*Storage:* `beat-audio` bucket (Beat Log's librosa uploads, private, blanket `anon_all`); `beat-deliverables` bucket (Aug 5, Phase H — real sellable beat deliverable files/stems/zips, private, anon INSERT/DELETE only — deliberately no anon SELECT, since raw bytes here are Alex's paid IP, not disposable analysis samples; reads only via the service-role key inside `api/data-write.js`'s `bundle-deliverables` action, or a signed URL it hands out).
...
## 15. Galaxy Map Rivers XII-XVI retired — added Aug 26 2026 (G102)

Real architecture change, per Alex's own confirmed answer ("all 5 and yes too"): Rivers XII (API/Auth Layer), XIII (Skills), XIV (Oversight Docs), XV (Session Records/Backlog), and XVI (Dev Tooling) are now marked **deprecated/merged** in `scripts/graphify_river_group.py`'s new `RIVER_RETIRED` dict — not deleted, since real code and docs still cite them by name (a live pointer, never a silent dangling reference).

Real evidence for exactly these 5, no others: all 5 always had **zero** real `rpgace_core.js` modules (`RIVER_MODULES[12..16] == []`, confirmed by direct query) — every one of them was a role-DESCRIPTION of a Total-systems category, never a river of real app code the way Rivers I-XI/XVII are. That real category is now represented at finer grain by the Galaxy Map's own L0 Infra/Inter bubble system (built Aug 25 2026, G77-G100), which covers the same real facts per-actor instead of one river-wide note:

- River XII → Supabase's own Infra/Inter system, plus each of the 6 connector L0 units (Oracle, Composio, Jina AI, Last.fm, librosa, n8n, Whisper), each showing exactly which real calls route through it.
- River XIII → the Skills L0 unit's own real per-skill Infra/Inter breakdown (`galaxy_map_skill_network.html`).
- River XIV → the Oversight Docs L0 unit (`galaxy_map_oversight_sync.html`).
- River XV/XVI → Orchestrator CC's own unit, an honestly-flagged **partial** successor (real Total-systems dispatch history covers River XV; the CC session that runs the dev-tooling scripts covers River XVI) — no L0 unit is a clean 1:1 match for either, stated plainly rather than force-mapped.

**What actually changed, mechanically**: a new `river_retirement_note_html(rnum)` helper (single shared source, rule 8) renders a real "⚠️ Retired — see instead" banner with live links to each river's real successor page(s). Wired into every real render site that shows a retired river's identity to a reader: Level 1's ring (a 🚫 badge on the node + a banner in its legend row), Level 2's per-river section (a full banner under the river title), the Logic Dimension page's per-river passages tab, and each retired river's own Obsidian vault note (a new `retired: true` frontmatter field + a callout block). A real, unrelated bug found and fixed in passing while touching the exact same frontmatter field: `graphify_to_obsidian.py`'s `carries_data_flow = num <= 11` predated the Aug 18 River-v2 split and silently mis-flagged River XVII (a real, module-bearing app-code river) as not carrying data flow — the same "stale hardcoded river-count boundary" bug class `TOTAL_ZONES` already taught this project once; fixed by deriving the flag from `RIVER_RETIRED` itself so it can never drift out of sync with the retirement list again.

**Rivers 1-11 and 17 are explicitly NOT touched** — they all have real modules and are the real app-code rivers this whole hierarchy exists to describe; a river having its own L0-unit facet elsewhere (Oracle/Supabase both appear as both a live river-adjacent module set AND an L0 unit) is a real additional lens, never a reason to retire a river that still has real code in it.

Full pipeline (all 18 Galaxy Map page scripts + the Obsidian export + `obsidian_vault_to_html.py`) regenerated and re-verified (idempotency, link-integrity, tag-balance, headless-Chromium) in the same pass. `minotaur_map.html`'s "Rivers XII-XVI — The Total System Gateway" section and `interconnection_map.md`'s own Total-systems paragraph both carry a matching real note, per each doc's own update rule (a real wing change for minotaur_map.html; a real Tier-b drift fix for interconnection_map.md).
