...
(function (global) {
  ...
  /* Bridge: route window globals through RPGACE.STATE accessors (with defaults) */
  function bridgeGlobal(globalName, getter, setter) {
    try {
    ...
    } catch (e) { /* already defined — non-fatal */ }
  }

  bridgeGlobal('_dailyDate',
    function()  { return R.STATE.dailyDate; },
    function(v) { R.STATE.dailyDate = v; });

  bridgeGlobal('_calWeekStart',
    function()  { return R.STATE.weekStart; },
    function(v) { R.STATE.weekStart = v; });

  bridgeGlobal('_calMonthDate',
    function()  { return R.STATE.monthDate; },   /* defaults to new Date() */
    function(v) { R.STATE.monthDate = v; });

  bridgeGlobal('_pendingSchedAgenda',
    function()  { return R.STATE.pendingSched; },
    function(v) { R.STATE.pendingSched = v; });
...
/* ================================================================
   YOUTUBE ORACLE MODULE — Step 9
   8-command panel pre-filled for @AceSanyaBeats
   Injected into Oracle tab via page:show hook
================================================================ */
...
RPGACE.register('youtubeOracle', {
  ...
  COMMANDS: [
    {
      ...
      prompt: 'Analyse this YouTube niche for @AceSanyaBeats:\nCHANNEL: @AceSanyaBeats\nNICHE: FL Studio beats / UK hip hop production tutorials\nAUDIENCE: Aspiring producers aged 18-35 wanting pro-sounding beats\nFREQUENCY: 2x per week\n\nReturn:\n1. Niche saturation score (1-10) with explanation\n2. Top 3 direct competitors and what makes them strong\n3. 3 underserved sub-niches with lower competition\n4. Monetisation potential ranking (AdSense / beats / courses / merch)\n5. Your single strongest differentiation angle\n\nBe specific and honest. Avoid generic advice.',
    },
    {
      icon: '🏛',
      label: 'Channel Identity Builder',
      prompt: 'Build the channel identity for @AceSanyaBeats:\nNICHE: FL Studio beats / UK hip hop production tutorials\nAUDIENCE: Aspiring producers aged 18-35\nUNIQUE ANGLES: Russian/French/London cultural perspective, working hospitality shifts while building independently\n\nReturn:\n1. 3 channel tagline options (under 10 words each)\n2. My Unique Mechanism — the one thing I teach that no one else teaches exactly this way\n3. Brand voice in 3 words\n4. Positioning statement (one sentence)\n5. The visual identity direction that matches this (colour palette, thumbnail energy, font personality)\n6. What to avoid — the clichés that would make this channel look like everyone else',
    },
    {
      icon: '📅',
      label: '90-Day Content Machine',
      prompt: 'Build a 90-day compound view strategy for @AceSanyaBeats:\nNICHE: FL Studio beats / UK hip hop production tutorials\nAUDIENCE: Aspiring producers aged 18-35\nFREQUENCY: 2x per week (8 videos per month, 24 total over 90 days)\n\nReturn:\n1. The 3-4 content pillars that should make up the 90 days\n2. Upload schedule with pillar rotation (not just dates — the logic behind the order)\n3. Ratio of evergreen vs trending content and why\n4. Which video type to lead with as video 1 and why\n5. How videos compound on each other — internal linking and series strategy\n6. The one video format I should test in month 2 based on my niche\n7. What "good traction" looks like at day 30 / 60 / 90 for this channel size',
    },
    {
      icon: '📝',
      label: 'Script Writer (Hook → CTA)',
      prompt: 'Write a complete YouTube script for @AceSanyaBeats:\nCHANNEL: @AceSanyaBeats\nNICHE: FL Studio beats / UK hip hop production tutorials\nAUDIENCE: Aspiring producers aged 18-35\nFORMAT: Tutorial showing FL Studio screen with on-camera moments\n\nScript must include:\n- HOOK (first 10 seconds): show the payoff immediately — what they will be able to do by the end\n- PROBLEM (30 seconds): why most producers get this wrong\n- SOLUTION (main body, step-by-step, FL Studio specific, with scene notes showing when to cut to screen)\n- PATTERN INTERRUPT every 90 seconds — a question, a surprising fact, or a reframe\n- CTA (final 20 seconds): subscribe prompt + tease the next video\n\nScene notes format: [SCREEN: show X] or [CAM: say Y to camera]\n\nTOPIC: [Tell me your video topic or I will suggest one based on your niche and current trending search terms]',
    },
    {
      icon: '🖱',
      label: 'Title + Thumbnail Optimizer',
      prompt: 'Generate 10 title options and thumbnail concepts for @AceSanyaBeats:\nCHANNEL: @AceSanyaBeats\nNICHE: FL Studio beats / UK hip hop production tutorials\nAUDIENCE: Aspiring producers aged 18-35\n\nFor each title:\n- CTR score (1-10) with one-line reason\n- Category: Curiosity Gap / Number / Transformation / How-To / Controversy\n\nFor each thumbnail concept:\n- Main image description (what the viewer sees)\n- Text overlay (3-5 words max)\n- Colour scheme\n- Emotion it triggers in 0.5 seconds\n\nThen rank your top 3 title-thumbnail combinations as a package.\n\nTOPIC: [Describe your video topic and I will generate all 10. Or I can suggest a topic from what is currently underserved in your niche.]',
    },
    {
      icon: '🔬',
      label: 'YouTube Algorithm Audit',
      prompt: 'Audit @AceSanyaBeats for YouTube algorithm performance:\nNICHE: FL Studio beats / UK hip hop production tutorials\nAUDIENCE: Aspiring producers aged 18-35\nFREQUENCY: 2x per week\n\nReturn a structured audit covering:\n1. CTR OPTIMISATION: what my titles and thumbnails need to do differently\n2. WATCH TIME: how to structure videos so viewers finish them (retention architecture)\n3. SESSION TIME: how to keep viewers on my channel after one video (end screen strategy, card strategy, playlist logic)\n4. UPLOAD CONSISTENCY: what happens algorithmically if I miss a week\n5. COMMUNITY SIGNALS: comment strategy that actually triggers algorithm signals\n6. SHORTS STRATEGY: should I post Shorts for this niche, and if so how many per week\n7. THE SINGLE BIGGEST MISTAKE music production channels make with the algorithm\n\nBe specific to the FL Studio / type beat / UK hip hop niche. Not generic YouTube advice.',
    },
    {
      icon: '🧠',
      label: 'Audience Mind Reader',
      prompt: 'Analyse the audience psychology for @AceSanyaBeats:\nAUDIENCE: Aspiring producers aged 18-35 wanting pro-sounding FL Studio beats\nCONTENT: UK hip hop and drill production tutorials\n\nReturn:\n1. Their 5 biggest frustrations (what keeps them stuck and frustrated at 2am)\n2. Their 3 core desires (what they actually want to feel, not just achieve)\n3. The exact language they use when describing their problem (phrases, search terms, how they talk about it in comments and Reddit)\n4. Content ideas that speak directly to each pain point — one idea per frustration\n5. The emotional journey from first video → subscriber → buyer\n6. What makes them click away in the first 30 seconds\n7. The one emotional trigger that makes this audience share content\n\nBase this on what you know about the music production / type beat / FL Studio creator community. Be precise and avoid generic creator audience analysis.',
    },
    {
      icon: '🔥',
      label: 'Viral Hook Generator (50)',
      prompt: 'Generate 50 viral hooks for @AceSanyaBeats YouTube content:\nNICHE: FL Studio beats / UK hip hop production tutorials\nAUDIENCE: Aspiring producers aged 18-35\nCHANNEL: @AceSanyaBeats\n\nReturn 50 hooks split into 3 categories:\n\nA) TITLE HOOKS (18 hooks) — for YouTube video titles. Must be specific to FL Studio and beat-making. No generic hooks.\n\nB) THUMBNAIL TEXT HOOKS (17 hooks) — 3-5 words maximum that stop the scroll. High contrast language. Producer-specific.\n\nC) FIRST-10-SECONDS SPOKEN HOOKS (15 hooks) — one sentence said on camera or in voiceover that forces them to stay. Opens a loop they need closed. Specific to music production.\n\nAfter the 50 hooks, give your top 5 picks across all categories and explain why they would outperform the others for this specific niche.',
    },
  ],
  ...
  /* ── Inject 🎬 button into Oracle tab ────────────────── */
  _injectButton: function() {
    if (document.getElementById('yt-oracle-btn')) return;
    var self = this;

    /* Find the Prod. Oracle button to anchor near it */
    var anchor = document.querySelector('[onclick*="toggleProdOraclePanel"], [onclick*="ProdOracle"], #prod-oracle-btn');
    if (!anchor) {
      /* Fallback: look for any button row near the chat input */
      anchor = document.querySelector('.chat-quick-actions, .oracle-actions, #chat-actions');
    }
    if (!anchor) return;
    ...
    btn.style.cssText = [
      ...
      'color:rgba(255,120,120,0.9);border-radius:6px;padding:6px 12px',
    ...
    btn.onmouseover = function() {
      btn.style.background = 'rgba(255,0,0,0.14)';
      btn.style.borderColor = 'rgba(255,80,80,0.45)';
    };
    btn.onmouseout = function() {
      btn.style.background = 'rgba(255,0,0,0.08)';
      btn.style.borderColor = 'rgba(255,80,80,0.25)';
    };
  ...
  /* ── Open command panel ──────────────────────────────── */
  openPanel: function() {
    var existing = document.getElementById('yt-oracle-panel');
    if (existing) { existing.remove(); return; } /* toggle */

    var self = this;
    var panel = document.createElement('div');
    panel.id = 'yt-oracle-panel';
    panel.style.cssText = [
      'position:fixed;top:0;right:0;width:min(380px,100vw);height:100vh',
      'background:#0c0c16;border-left:1px solid rgba(255,80,80,0.15)',
    ...
    var cmds = this.COMMANDS.map(function(cmd, i) {
      return '<button onclick="RPGACE.modules.youtubeOracle.run(' + i + ')" style="'
        + 'display:flex;align-items:center;gap:10px;width:100%;padding:11px 16px;'
        + 'background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);'
        + 'border-radius:7px;cursor:pointer;text-align:left;color:rgba(226,226,236,0.82);'
        + 'font-family:Rajdhani,sans-serif;font-size:12px;font-weight:600;letter-spacing:0.5px;'
        + 'transition:all .15s;margin-bottom:6px;'
        + '" onmouseover="this.style.background=\'rgba(255,80,80,0.08)\';this.style.borderColor=\'rgba(255,80,80,0.2)\'"'
    ...
    panel.innerHTML = [
      /* Header */
      '<div style="background:rgba(255,40,40,0.06);border-bottom:1px solid rgba(255,80,80,0.12);',
        ...
        '<div>',
          '<div style="font-size:9px;font-weight:700;letter-spacing:3px;text-transform:uppercase;',
            'color:rgba(255,100,100,0.65);margin-bottom:3px">YouTube Oracle</div>',
          '<div style="font-size:14px;font-weight:700;color:#E2E2EC">@AceSanyaBeats</div>',
          '<div style="font-size:10px;color:rgba(226,226,236,0.35);margin-top:2px">',
            'FL Studio · UK Hip Hop · 2x per week',
          '</div>',
        '</div>',
        '<button onclick="document.getElementById(\'yt-oracle-panel\').remove()" ',
          'style="background:none;border:none;color:rgba(226,226,236,0.3);cursor:pointer;',
          'font-size:18px;line-height:1;padding:4px">&times;</button>',
      '</div>',
      /* Commands */
      '<div style="flex:1;overflow-y:auto;padding:16px">',
        '<div style="font-size:9px;font-weight:700;letter-spacing:2px;text-transform:uppercase;',
          'color:rgba(226,226,236,0.3);margin-bottom:12px">8 COMMANDS · PRE-FILLED FOR YOUR CHANNEL</div>',
        cmds,
      '</div>',
      /* Footer */
      '<div style="border-top:1px solid rgba(255,255,255,0.05);padding:12px 16px;flex-shrink:0">',
        '<div style="font-size:10px;color:rgba(226,226,236,0.3);text-align:center">',
          'Output goes to Oracle chat · Claude Sonnet 4.6',
        '</div>',
      '</div>',
    ].join('');
  ...
  /* ── Run a command → send to Oracle chat ─────────────── */
  run: function(idx) {
    ...
    /* Inject into Oracle chat input and send */
    var input = document.getElementById('chat-input') || document.querySelector('textarea[placeholder*="Oracle"]');
    if (input) {
      ...
      RPGACE.utils.toast('🎬 ' + cmd.label + ' — running...', 'rgba(255,100,100,0.9)', 2500);
    } else {
      /* No input found — copy to clipboard as fallback */
      RPGACE.utils.copy(cmd.prompt);
      RPGACE.utils.toast('🎬 ' + cmd.label + ' — copied to clipboard, paste in Oracle', 'rgba(255,100,100,0.9)', 4000);
    }
  },
