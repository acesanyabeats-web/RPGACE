...
- **Phylum Path Phase 2 (parked, not built)** — dedicated nav tab, Linnaean drill-down browsing replacing the current flat list, scope/zoom sibling browsing, pre-built per-branch articles, direct-from-chat placement, and "Circles" (rabbit-hole nav, originally deferred July 8 pending a Research-tab declutter session, now folded into this vision). Full spec not written yet — see `/root/.claude/plans/woolly-watching-lamport.md`.
