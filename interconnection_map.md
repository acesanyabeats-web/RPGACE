...
**Spine table: `content_productions`.** Three real creation paths write it — `contentRepurpose.createEntry()`, `conidPot`'s "Activate ConID", and `beatLog._submit()` — and every ConID card button updates it (`updateEntry()` — status/title/details). Read by the Dashboard ConID widget and the Oracle bar's active-ConID context. A `content_type` discriminator column (`'tutorial'` | `'music_video'`) distinguishes the two genuinely different workflows the same table serves; the Council-of-5 verdict was one shared table plus a discriminator, not a split, because all three creation paths have near-identical mechanics. Status flow is the shared `Idea → Scripted → Filmed → Edited → Posted → Analysed` column; `music_video` rows *display* Alex's own 4-step model (Beat Logged / Visual Treatment / Video Generation / Captions Ready) as label overrides on that same column — display-only, zero schema change. `contentProductionLive._openProductionPanel` branches its Phase 1-3 copy on `content_type`, and the `music_video` variant carries a hop button into Video Pipeline.
...
**🎬 Auto Visual Treatment Doc (F18) + the director picker:** an optional Beat Log checkbox waits for the Oracle concurrency guard to clear, then auto-fires Visual Oracle's treatment-doc structure with real beat data. The same command is reachable directly from a ConID card ("🎬 Generate Visual Treatment", pulling that row's real BPM/key/mood/genre and auto-saving back to it). Its director-reference field is a real picker (`visualOracle._showDirectorPicker`) over the 50-row `f14_filmmaker_library` — dropdown + a "View Style" popup surfacing that director's real taxonomy data + a free-text inspiration box — wired into both call sites. The treatment prompt's deliverables include a **Character Reference Block** (a fixed physical description of any recurring character) — a provider-independent answer to the fact that no image/video generation provider guarantees character consistency across separately-generated scenes.
