...
- **`phylumPath.PHYLUM_NUM` generalization — next concrete step.** Needed before Phylum Path (insight placement, auto-detect, nav-tab drill-down, fusion-link search) can cover Percussio or any second phylum. Not a one-line flip — see the Percussio paragraph above.
