...
**● Phylum 1 tree built out from the 90-term jargon sweep** — 32 new rows extending the 5 existing Orders plus one brand-new 6th Order ("Genre & Production Vocabulary" for UK-drill/FL-Studio-specific jargon that didn't fit the existing 5). Every one of the 90 terms lives under a real node now, grouped into coherent terminology-bucket leaves (e.g. "Cadence & Resolution Vocabulary" holds cadence/resolution/tension/i-iv-v/ii-v-i/circle of fifths/pivot chord/modulation/progression) rather than 90 individual leaves. All new rows chained via real `parent_id` from creation (no repeat of the flat-lineage bug). Tagged `source: phylum_path_manual`; terminal nodes carry a placeholder `deep_content` flagging that full teaching content isn't generated yet, ready for Phylum Path's "Generate/Refresh Article" button.

**● Correction, same session: artist/track-specific nodes moved out of the tree, into Encyclopedia** — two leaves named specific artists ("Dave x Central Cee 'Our 25th Birthday' Type-Beat...", "Oracle Deep Dive: Endshpiel & Miyagi-Type Beats...") were flagged as violating the taxonomy's job — it should classify general, reusable concepts, not bake in one-off applied examples. Moved both into ▢ **`encyclopedia`** as real entries (`source: 'taxonomy_case_study'`), each `taxonomy_node_id`-linked back to the general concept node they illustrate ("Tempo, Melody, and Harmonic Structure in UK Rap Beats" and "Post-Soviet Melodic Traditions" respectively — same F7 linking mechanism, applied in the opposite direction: case study → general node, not general node → its own proposal). Both parent nodes flipped back to `node_type: 'leaf'` since they no longer have tree children. Left "CIS Melodic Trap Harmonic Style" alone — a regional/style descriptor, not a named artist, judgment call flagged for confirmation. Phylum 1 row count: 55 → 53.
