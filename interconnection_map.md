...
Every documentation update applies to all **six** Oversight docs plus Chronicles by default, not only when explicitly requested — same discipline as code changes, formalized for docs. **Oversight** = Patch Notes (`patch_notes.html`, full narrative + F-series roadmap), this Interconnection Map (structural touchpoints, standing sections updated in place, never a changelog), the Full Manual (`manual.html`, polished quick-reference), Taxonomy Map (`taxonomy_map.html`, queries `taxonomy_tree` live every load — only touched if its own code/columns change), System Flow Map (`system_flow_map.md`, Mermaid diagrams + built/not-built truth table), and the Minotaur Map (`minotaur_map.html`, the same flows as a spatial river metaphor — updated only when a wing is added). Chronicles (`system_updates`) is the seventh, live, in-app place the same work is recorded. See `CLAUDE.md`'s Oversight section for the durable version of this rule.
