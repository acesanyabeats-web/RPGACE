...
**Phylum Path generalized to multiple phyla (built July 17) — Percussio now enabled.** `phylumPath.PHYLUM_NUM` (a single hardcoded scalar threaded through ~10 places) is now `ENABLED_PHYLA: [1, 2]` + mutable "currently active phylum" UI state. `isEnabled()` checks list membership instead of equality — `proposeLineage()`/`silentPropose()`'s existing routing check automatically covers Percussio with no separate change. New `_switchPhylum()`/`_renderPhylumSwitcher()` — a pill row (one per enabled phylum) in both the side panel and the nav-tab page — updates the active phylum and re-fetches whichever view is actually visible. Auto-detect (`_checkLastResponse`) now checks every enabled phylum's score instead of only Phylum 1's, opening the panel pre-scoped to whichever phylum's keywords matched. Extending to a third phylum going forward is just adding a number to `ENABLED_PHYLA` once that phylum clears framework steps 1-4 — no further UI rework needed.
...
- **`phylumPath.PHYLUM_NUM` generalization — shipped July 17.** See the "Phylum Path generalized" paragraph above. Not yet hand-tested.
