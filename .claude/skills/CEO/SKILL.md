...
- **`dormant`** — a NEW, fully-derived (not stored) state: the active plan's own `ceo_plan_items` are all green (real, checkable — `ceoMode._fetchState()` auto-flips the plan's `status` to `'complete'` the moment this becomes true) AND `future_integrations.html` (every non-green `ceo_plan_items` row, across every plan) is completely empty — nothing real left to ground against. The live `🎯 CEO` toggle pill shows this as a real, visually distinct `💤 CEO: dormant` state (blue-grey, not the grey `paused` uses) precisely because it means something different — Alex doesn't need to do anything for this to end; the very next real plan or Future Integrations item, from ANY source, makes the next render read `active` again automatically, with no explicit "reactivate" step. Clicking a dormant pill explains this rather than pretending to toggle it (it isn't a manual state).
Real implementation: `RPGACE.register('ceoMode', {...})` in `rpgace_core.js` — `_fetchState()` computes this derivation live on every render, `_deriveDisplay()` is the shared logic both the pill and any future chat-facing `/stopCEO`/`/ResumeCEO` status check should read from (one shared implementation, rule 8).
