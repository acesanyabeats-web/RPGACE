...
- **`/perspective`** — the evidence-grounded first-person self-report method; backs the topology the Galaxy Map draws (`perspective_reports`, 21 rows so far).
...
**What's real and shipped so far** (see `ceo_plan_items` for the live, checkable state — this list is a snapshot, not the source of truth): a 4-level drill-down (Level 0 galaxies → Level 1 rivers → Level 2 modules/dashboard-cards → Level 3 function-call chains, with real rank-band sub-pages for crowded modules), a real 4-signal intra-river connectivity detector, verified crossing-reduction at every level, a real permanent "Alex" human-actor bubble at every level (evidence-gated, precision-tuned twice after real feedback), real cross-module/cross-river "backdoor" and "next function" navigation stubs, and a confirmed, named structural limit (the whole pipeline currently only reads `rpgace_core.js` — zero visibility into `main.js` or `RPGACE.hooks`) that the next real phase (G15, Level 4) is scoped to close.
