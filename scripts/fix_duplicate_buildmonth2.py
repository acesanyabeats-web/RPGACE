...
new = "// (dead duplicate buildMonthSlots removed - old goal-tracker version, superseded by the real shift-calendar version later in this file)"

if old in src:
    fixed = src.replace(old, new, 1)
    open('main.js', 'w', encoding='utf-8').write(fixed)
    print("PATCHED: dead duplicate buildMonthSlots removed successfully")
else:
