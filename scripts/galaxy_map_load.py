...
def build_bubble_view(n_boot, n_pagenav, n_click, n_events):
    """GMR-6 (Sep 16 2026) — the real, evidence-gated '⏳ Load' hub
    bubble this page's own note footer already named as a deferred
    follow-on ("mirroring the existing Oracle/Composio bubble pattern
    ... a real, separate follow-on — not built in this pass"). Pure
    rendering layer over the SAME 4 real counts the tab sections above
    already compute and print (R22 — bubble follows table, never a
    second source). Each leaf links to its own real tab section by the
    same #cat-* anchor id the tab-picker already uses — no new anchor
    scheme invented. Real, honest scope note: this is the aggregate
    4-category summary specifically, not the deeper real per-module
    "which categories does module X have evidence in" conjoined view
    the original note also named — that stays its own real, separately
    scoped future item (44 modules x 4 categories is a materially
    bigger real build, not rushed into this pass)."""
    hub = dict(icon='⏳', label='Load Dimension', color='#2ABFB0')
    cats = [
        ('cat-boot', '⏱️', 'Boot-Time', n_boot),
        ('cat-pagenav', '📄', 'Page-Nav', n_pagenav),
        ('cat-click', '🖱️', 'Click', n_click),
        ('cat-events', '📡', 'Events', n_events),
    ]
    leaves = [
        dict(id=cat_id, icon=icon, label=label, sub=f'{n} real trigger(s)',
             color='#2ABFB0', href=f'#{cat_id}')
        for cat_id, icon, label, n in cats if n > 0
    ]
    ...
    return (
        ...
        + '<div class="lp-bubble-block"><h3>⏳ The 4 real load-trigger categories, at a glance</h3>'
        '<p class="lp-bnote">Click a category to jump to its own table below. '
        'Same real counts each tab already prints — this is a pure rendering layer over them, never a second source.</p>'
        f'{bubble}</div></div>'
    )
...
<style>
  ...
  *{{box-sizing:border-box;margin:0;padding:0}}
  ...
  .lp-bubble-block{{max-width:1000px;margin:0 auto 26px;padding:0 24px}}
...
<div class="note">
  ...
  Real update, Sep 16 2026 (GMR-6): the "⏳ Load" hub bubble above is now real — a pure rendering layer over the same 4
  category counts the tabs below already compute, mirroring the Oracle/Composio bubble pattern. Real, honest scope limit,
  still open: the deeper per-module "which categories does module X have evidence in" conjoined view is a materially bigger
  real build (44 modules x 4 categories) and stays a real, separately scoped future item, not rushed into this pass.
...
def main():
    ...
    n_pagenav = sum(len(compute_page_nav_triggers(m)) for m in LEVEL3_MODULES)
    n_click = sum(len(v) for v in click_triggers.values())
    n_events = len([e for e in event_edges if e[2] != 'page:show'])
