...
new = """      var isContentIdea = text.includes('tutorial') || text.includes('teach') || text.includes('how to') ||
      ...
      var isSocialIdea = text.includes('instagram') || text.includes('tiktok') || text.includes('reel') ||
        text.includes('caption') || text.includes('hook') || text.includes('content') || text.includes('post') ||
        text.includes('platform') || text.includes('youtube');
...
# Also add dedup after the forEach closes
old2 = """        callback(confirmed, suggested, []);
      });
    } else {
      callback(confirmed, suggested, []);
    }"""

new2 = """        // Deduplicate by phylum number
        var seenNums = {};
...
if old in src:
    ...
    print("Fix 1 ERROR - showing context:")
    idx = src.find('allPhyla.forEach')
