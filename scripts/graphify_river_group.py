...
EXTERNAL_CONNECTORS = [
    ...
    {'name': 'fal.ai', 'status': 'live (external repo)', 'tested': False, 'via': 'a real fal.ai key set up directly in the OpenMontage repo, Sep 16 2026',
     'bridges_to': 'runs inside the OpenMontage galaxy itself, not a separate bridge — same relationship as FFmpeg',
...
EXTERNAL_RIVER_LINKS = [
    ...
    {'name': 'fal.ai', 'rivers': [11],
     'via': "Same relationship as FFmpeg — runs inside OpenMontage's own repo, reached only via River XI's OpenMontage handoff. Real working provider as of Sep 16 2026 (GMR-1), unlike OpenArt (deferred, no real tool integration)."},
...
DIMENSION_PAGES = [
    ...
    # this registry: Alex's own ratified scope retires the Logic
    # Dimension as a standalone destination. Its 21 curated decision/
    # logic entries are now dispersed onto their real home objects
    # (galaxy_map_decision_matrix.build_module_decisions_html() on
    # Current(L3), build_river_decisions_html() on Level 2), and the
    # river-to-river/connector/skill passages it rendered were always a
    # SECOND presentation of data Level 2's own river legend already
    # draws from the same RIVER_FLOWS/FLOWS_IN/LINKS_BY_RIVER/
    # ALL_SKILLS/SKILL_SECONDARY_RIVER tables (verified by direct read of
    # galaxy_map_module.py's own imports, not assumed) — so nothing real
    # is lost by de-registering it. The file itself stays on disk with a
    # real retirement banner (same "deprecated, not deleted" precedent as
    # RIVER_RETIRED and galaxy_map_hub.html), so no existing bookmark
    # 404s. Level 6 (the exhaustive branch ledger) is deliberately NOT
    # touched — it stays a registered, link-out-only reference, which is
    # the other half of G111's own ratified scope.
    ...
    # G110 (Sep 1 2026) — local_server.py's real Galaxy Map identity.
    # Registered as ONE Inter dimension joining local_server.py +
    # Whisper + the direct Anthropic call, per Alex's own ratified
    # direction ("make a dimension joining three as an inter, so it all
    # one bubble migration with the logic attached") — deliberately NOT
    # 3 peer EXTERNAL_CONNECTORS entries, and deliberately excluding
    # librosa, whose very existence is still genuinely unconfirmed
    # (ceo_plan_items G114).
...
def inject_level_rail(html, current_file):
    ...
    html = collapse_methodology(html)
    return html


# ---------------------------------------------------------------------
# G113 (Sep 1 2026) — collapse the "how this doc is formed" methodology
# text, IN PLACE, collapsed by default.
#
# Alex's own ratified scope, verbatim: "no just a methodology that is in
# the doc to only be in the doc with context to only doc, just expand/
# contractable with contracted as default to remove clutter - rarely
# need this information unless im changing the oversight doc in context
# is working and extracting information itself, otherwise i just need
# the data." He explicitly REJECTED consolidating it onto one shared
# methodology page — it stays exactly where it already lives, just
# folded away.
#
# Real evidence for what qualifies, gathered before writing this:
#   * Every one of the 21 real galaxy_map_*.html pages carries exactly
#     ONE `<div class="note">` footer, and every one of them is pure
#     "Generated by scripts/X.py, reusing Y, mapping rules:
#     system_map_spec.md" methodology — the literal "how this doc is
#     formed" text Alex named. Verified by direct scan: 21/21 pages, and
#     zero of them contain a nested <div>, so the naive open-tag-to-
#     first-close-tag wrap below is genuinely safe (checked, not assumed).
#   * galaxy_map_hub.py's "How to read this map" primer is the one other
#     real instance; it is wrapped at its own source (build_primer()),
#     because it sits mid-page with its own heading rather than being a
#     uniform footer.
#
# Explicitly NOT in scope, per the same ratified answer: page-specific
# INTRO paragraphs (galaxy_map.py's own L0 hero text, each page's `.sub`
# lines). Those describe the CONTENT, not how the doc was generated —
# folding them away would hide the data Alex says he actually wants.
#
# Done as a shared post-process on the one hook every page script
...
def collapse_methodology(html):
    """Wrap this page's own `<div class="note">…</div>` methodology
    footer in a native `<details>`, collapsed by default (no `open`
    attribute). Idempotent by construction: if the page already carries
    a `doc-methodology` details block, or genuinely has no note footer,
    it is returned untouched — so re-running the pipeline can never
    nest one inside another."""
    if 'class="doc-methodology"' in html or '<div class="note">' not in html:
        return html
    start = html.index('<div class="note">')
    end = html.index('</div>', start) + len('</div>')
    block = html[start:end]
    wrapped = ('<details class="doc-methodology">'
