...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>RPGACE — Galaxy Map (Logic Dimension, retired)</title>
<style>
  :root {{ --bg:#050508; --gold:#C9A84C; --text:#E2E2EC; --dim:#8a8a9a; --purple:#9B59B6; }}
  *{{box-sizing:border-box;margin:0;padding:0}}
  body{{background:radial-gradient(ellipse at 50% 30%, #12101a 0%, #050508 70%);color:var(--text);font-family:'Segoe UI',system-ui,sans-serif}}
  ...
  .hero h1{{font-family:Georgia,serif;font-size:26px;color:#fff;margin-bottom:8px}}
  .hero p{{color:var(--dim);font-size:12px;max-width:900px;margin:0 auto}}
  .page-retired{{max-width:900px;margin:14px auto 40px;padding:12px 16px;border-left:3px solid #cc4a4a;
    background:rgba(204,74,74,.10);border-radius:6px;font-size:11.5px;line-height:1.7;color:#c8c8d8}}
  .page-retired b{{color:#fff}}
  .page-retired a{{color:var(--gold)}}
  .page-retired code{{background:rgba(255,255,255,0.06);padding:1px 5px;border-radius:4px;font-size:10.5px}}
...
<div class="page-retired">
  ...
  every pipeline run (~60KB of real content that had already been a pure second presentation of <a href="galaxy_map_module.html">Level 2</a>'s own
  <code>RIVER_FLOWS</code>/<code>FLOWS_IN</code>/<code>LINKS_BY_RIVER</code>/<code>ALL_SKILLS</code>/<code>SKILL_SECONDARY_RIVER</code> data since G111 — real duplicate
  bytes on disk for a page already delisted from navigation. Now a real, small ~13KB stub (mostly the shared nav/plan-overlay JS every page carries, not the retired content) — down from ~60KB; the generator functions that built the old full page
  (<code>build_river_section()</code>/<code>build_river_passages()</code>) are kept in this file, unused, rather than deleted, per this project's own
  archive-never-delete convention — a future session could still resurrect the full view from them if a real need for it ever returns.
</div>
<div class="note">
  Generated by <code>scripts/galaxy_map_logic_dimension.py</code>. Mapping rules: <code>system_map_spec.md</code>.
</div>
</body>
</html>
"""
...
def main():
    ...
    print(f"Wrote {OUT} — real ~13KB retirement stub (P0, Sep 24 2026, down from ~60KB), replacing the old full 17-river regeneration.")
