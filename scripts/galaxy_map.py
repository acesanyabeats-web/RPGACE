...
def build_svg():
    ...
    def edge(x1, y1, x2, y2, itype, tested=True, offset_mult=1, r1=0, r2=0, from_label=None, to_label=None):
        ...
        # P0 edge evidence substrate (Sep 24 2026): itype is already the
    ...
    if 'openmontage_cc' in galaxy_pos:
        ...
        if om_conn and om_conn['name'] in connector_pos:
            ...
            edges_svg.append(_curved_edge(oox, ooy, omx, omy, '#E25454', real=True, dashed=True, r1=14, r2=34,
                                           from_label='OpenMontage', to_label='OpenMontage CC', kind='dispatch_trigger'))
...
<details class="rollup-details">
  <summary>🔭 3 real Level-0 tiers, listed by name (GMP-C, Sep 16 2026)</summary>
  <div class="note" style="max-width:1000px">
    <p>L0 has always held 3 genuinely different kinds of unit — this labels them explicitly for the first time. Nothing here is new data; every row already renders elsewhere on this page (the diagram above, or its own bubble/panel) — purely an organizational summary, collapsed by default so it doesn't crowd the real content above (Sep 24 2026, real Alex ask). Click any row to jump to that same real unit.</p>
    {tiers_block}
  </div>
</details>
...
(function() {{
  ...
  // G83/G91 (Aug 25 2026) — the real L0-unit -> bubble-system redirect
  // map, hoisted to this outer scope (was declared per-click inside the
  // card handler below) so the table-view row handlers further down
  ...
  cards.forEach(function(c) {{
    c.addEventListener('click', function() {{
      ...
      // 3 registered tables, same evidence shape) — each destination
      // page was independently checked to genuinely supersede its own
      // L0 inline facet list before being wired in here (see
      // UNIT_BUBBLE_SYSTEM above). The remaining units (OpenMontage CC/
      // Graphify CC/Skills/Alex/RPGACE Architecture) keep the inline
      // facet panel unchanged until each has a real destination of its
      // own — OpenMontage CC in particular has no rpgace_core.js
      // footprint of its own to drill into (a separate repo/session).
      if (UNIT_BUBBLE_SYSTEM[uid]) {{
        window.location.href = UNIT_BUBBLE_SYSTEM[uid];
        return;
      }}
      currentUnit = uid;
      currentKind = null;
  ...
  document.querySelectorAll('th.unit-rowhead').forEach(function(th) {{
    th.addEventListener('click', function() {{
      ...
      // G91 continuation (Aug 25 2026) — a real bug found this same
      // day: for a unit wired into UNIT_BUBBLE_SYSTEM, card.click()
      // below already navigates the page away — clicking through the
      // map view's own toggle/panel machinery on the way out is not
      // just wasted work, it threw "Cannot read properties of
      // undefined (reading 'facets')" (renderDims() reading
      // DATA.units[currentUnit] when currentUnit was never set, since
      // the redirect returns before setting it). Fixed by short-
      // circuiting straight to navigation for a redirect unit, the
      // same real jump the card's own handler would produce.
      if (UNIT_BUBBLE_SYSTEM[uid]) {{
        window.location.href = UNIT_BUBBLE_SYSTEM[uid];
        return;
      }}
  ...
  document.querySelectorAll('#unit-facets tr.uf-row[data-unit]').forEach(function(tr) {{
    tr.addEventListener('click', function() {{
      ...
      // G91 continuation (Aug 25 2026) — same real fix as the matrix
      // row-header handler above: a redirect unit's facet-table row
      // must navigate straight away too, never fall through to
      // kind/dim-click logic that assumes the inline panel opened.
      if (UNIT_BUBBLE_SYSTEM[uid]) {{
        window.location.href = UNIT_BUBBLE_SYSTEM[uid];
        return;
      }}
