...
(function() {{
  ...
  cards.forEach(function(c) {{
    c.addEventListener('click', function() {{
      ...
      // view). G91 continuation (same day) generalizes this to the
      // 2 more units Alex confirmed have real river/module-shaped
      // evidence to build the same shape for: External AI (Oracle-call
      // evidence) and Oversight Docs (table-touch evidence, scoped to
      // what oversight docs themselves touch) — each destination page
      // was independently checked to genuinely supersede its own L0
      // inline facet list (galaxy_map_externals.html already carries
      // every External-AI actor's role/status/destination via its
      // existing 3 tabs; galaxy_map_oversight_sync.html's new Shared
      // Infrastructure tab now restates the real per-{{doc,table}} rows
      // too, not just the river/module drill-down) before being wired
      // in here. The other 6 units keep the inline facet panel
      // unchanged until each has a real destination of its own.
