 ...
 * 21% comment lines) + main.js (257KB/75KB gzipped) together account for
 * ~1.38MB of blocking JS parsed+executed before the app is interactive —
 * Alex reported this still measuring ~12,000ms on his laptop even after the
 * (correctly-scoped, but insufficient alone) cache-header/preconnect fixes.
 *
 ...
 * OUTPUT files are what actually get committed and served.
 *
 * SAFETY, real and verified, not assumed:
 *   - mangle: false, ALWAYS. index.html has 93 inline onclick="fnName(...)"
 *     HTML attributes referencing top-level global functions in main.js/
 *     rpgace_core.js. A minifier is a plain, per-file static analyzer — it
 *     cannot see those references living in a SEPARATE HTML file, so
 *     renaming would silently break every one of those 93 buttons with a
 *     runtime "X is not defined" error only visible in DevTools. Verified
 *     directly (Aug 11 2026): with mangle:false, every one of the 47
 ...
 *
 * MUST re-run whenever rpgace_core.js or main.js changes, in the SAME
 * commit — same discipline as the existing "bump index.html's ?v= in the
 * same commit" landmine rule, now extended. A stale .min.js file being
 * served while the real source has moved on is a silent "nothing changed"
 * bug, same failure class as every other cache-staleness landmine in this
 * project. `node scripts/minify_client.js` regenerates both files.
 */
...
const TARGETS = ['rpgace_core.js', 'main.js'];
