...
- ENTRY/ORPHAN badges are computed from real in-degree over that same
  ...
  this page set (real count: len(PAGES), computed fresh at build time,
  never hardcoded — see the Sep 22 2026 fix note where this exact
  literal drifted out of sync with the real page count once before) —
  a page-grain 🏁 terminal claim would be false, so
...
PAGES = [
    ...
    {'file': 'galaxy_map_orchestrator_cc.html', 'label': 'Orchestrator CC', 'level': 'Dimension', 'kind': 'inter',
     'scope': '6-facet profile + 8 real dispatch rows', 'desc': 'G29, real Sep 15 2026 2-page split — Orchestrator CC\'s own real profile + shared dispatch history with OpenMontage CC via openmontage_jobs, on its own separate page.'},
    {'file': 'galaxy_map_openmontage_cc.html', 'label': 'OpenMontage CC', 'level': 'Dimension', 'kind': 'inter',
     'scope': '6-facet profile + 8 real dispatch rows', 'desc': 'G29, real Sep 15 2026 2-page split — OpenMontage CC\'s own real profile + shared dispatch history with Orchestrator CC via openmontage_jobs, on its own separate page.'},
...
<div class="note">
  Generated by <code>scripts/galaxy_map_hub.py</code> (G59, real /interrogation Aug 21 2026 — 4 forks, all recommended options confirmed, full record in <code>records/2026-08/galaxy_map_unified_gateway_spec_2026-08-21.txt</code>). {n_edges} real edges computed fresh at build time from every page's own actual cross-references (galaxy_map.html excluded as a target — it's a universal home breadcrumb, not a real flow relationship). Honest limitation: a page-grain 🏁 terminal badge is NOT shown here — real out-degree never hits zero across this {n_pages}-page set (pages cross-link for reference as well as drill-down), so a terminal claim would be false; the 🚪/🏁 convention stays accurate at the function grain (Level 3/6/Decisions/Zoom) where it was built. The deeper infra/inter retrofit — converting a genuinely-shared infra bubble into a real per-edge selector INSIDE each of these {n_pages} pages' own rendering — is G60, logged and deliberately not built this pass (phased build, per Alex's own confirmed choice).
</div>
<script>
(function() {{
