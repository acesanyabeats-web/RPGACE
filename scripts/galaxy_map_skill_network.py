...
invocation, grouped by the calling skill — the real, buildable
alternative to a 24-node/117-edge force-directed graph, which this
project's zero-npm-runtime rule has no library to lay out well by hand.
"""
import re
...
<div class="hero">
  ...
  <p>Real, distinct from the AI/UI/Backend dimension (G28/Skills page) — this is skill-CALLS-skill composition: {n_edges} real invocations across {n_skills} skills, mechanically detected from each skill's own SKILL.md prose (a real `/otherSkillName` mention), never invented. Click any invocation to see it named.</p>
</div>
...
<div class="note">
  ...
  this detector. Mapping rules: <code>system_map_spec.md</code>.
</div>
<script>
(function() {{
...
def main():
    ...
    print(f"Wrote {OUT} — {len(names)} skills, {n_edges} real invocation edges.")
